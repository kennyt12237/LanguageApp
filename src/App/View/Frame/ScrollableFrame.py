...
class ScrollableGrammarFrame(Frame):
    ...
    def __init__(self, master : Misc, data: json = None, name='grammar', **kwargs) -> None:
        ...
        self.canvas.configure(yscrollcommand=self.scrollbar.set, yscrollincrement=75)
        self.canvas.create_window((0,0), anchor="nw", window=self.grammarFrame, tags=self.GRAMMAR_FRAME)
    ...
    def onCanvasConfigure(self, event : Event) -> None:
        self.canvas.itemconfig(self.GRAMMAR_FRAME, width=self.canvas.winfo_width())
